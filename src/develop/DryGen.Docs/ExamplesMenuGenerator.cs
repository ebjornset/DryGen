...
public static class ExamplesMenuGenerator
{
    ...
    {
        var sb = new StringBuilder().Append("# ").AppendLine(DocsConstants.AutoGeneratedNotice).AppendLine("- name: dry-gen examples").AppendLine("  href: index.md");
        foreach (var exampleTemplateFile in Directory.GetFiles(examplesTemplatesDirectory).OrderBy(x => x))
        {
            ...
            sb.Append("- name: ").AppendLine(fileName.Replace('_', ' ').Replace('-', ' ')).Append("  href: ").Append(fileName.ToLowerInvariant()).AppendLine(".md");
        }
