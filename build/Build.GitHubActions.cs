...
[DotNetGitHubActions(name: "tag-version", AutoGenerate = false, OnWorkflowDispatchRequiredInputs = ["version"], InvokedTargets = [nameof(CiCd_TagVersion)], PublishArtifacts = false, ImportSecrets = [nameof(SonarToken)])]
[NugetPushGitHubActions(name: "release", OnPushTags = ["v*.*.*"], InvokedTargets = [nameof(CiCd_Release)], PublishArtifacts = false, ImportSecrets = [nameof(NuGetApiKey), nameof(SonarToken)], WritePermissions = [GitHubActionsPermissions.IdToken, GitHubActionsPermissions.Pages], EnvironmentName = "github-pages")]
